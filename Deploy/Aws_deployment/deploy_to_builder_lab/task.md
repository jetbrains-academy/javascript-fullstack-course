...
You will work in an AWS lab environment that provides pre-configured infrastructure including VPC, subnets, and ECS cluster.

### Provided AWS Lab Environment

You will work in a pre-configured AWS environment with:
...
- **Amazon ECR repositories** - For storing your Docker images.
...
1. **Build and test Docker images locally** - Verify containers work before pushing to AWS as you done in the previous task.
...
6. **Test the deployed application** - Verify all endpoints work in the cloud environment.

### Success criteria

- **Inventory API functional** - All backend routes work correctly.
- **Gateway service accessible** - HTTP requests reach the frontend through the ALB.
- **Service communication** - Frontend successfully forwards requests to the backend.
...
**Lab Setup**: Access the AWS lab environment through the [link](staging.academy.labs.jb.gg/lti/platform/resource/launch/platfromresource1/load). The lab includes a Cloud9 environment where you can run Docker commands and AWS CLI. All necessary permissions and infrastructure are pre-configured.
