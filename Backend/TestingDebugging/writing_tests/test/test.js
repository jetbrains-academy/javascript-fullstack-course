...
beforeAll(() => {
    let testFilePath = path.join(process.cwd(), 'Backend', 'TestingDebugging', 'writing_tests', 'backend',
...
test('Test file concat.test.js is pass', async () => {
    try {
        try{
            const result = await runJestFile('Backend/TestingDebugging/writing_tests/backend/__tests__/concat.test.js');
